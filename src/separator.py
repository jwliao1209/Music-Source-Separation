...
class Separator(nn.Module):
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        # Shape of spectrograms: (nb_samples, nb_channels, nb_bins, nb_frames, nb_sources)
    ...
    def seperate(self, audio: Tensor) -> tuple[Tensor, Tensor]:
        nb_sources = self.nb_targets
        nb_samples = audio.shape[0]
        ...
        for c in range(nb_channels):
            ...
            estimated_vocals.append(
                librosa.griffinlim(
                    ...
                    # win_length=self.wiener_win_len,
        ...
        return np.stack(estimated_vocals, axis=1), np.stack(estimated_nonvocals, axis=1)
...
def load_separator(
    ...
    model = OpenUnmix(
        input_mean=np.array(config.train_data_mean),
        input_scale=np.array(config.train_data_std),
    ...
    separator = Separator(
        ...
        output_spectrograms=output_spectrograms,
    ).to(device)
